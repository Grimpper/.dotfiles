...
This function should only modify configuration layer settings."
  (setq-default
   ...
   '(vimscript
     ...
     (dap :variables dap-lldb-debug-program '("/usr/bin/lldb-vscode"))
...
If you are unsure, try setting them in `dotspacemacs/user-config' first."
  (custom-set-variables '(spacemacs-theme-custom-colors'((bg1 . "#262b35"))))
)
...
before packages are loaded."
  ...
  (define-key evil-visual-state-map (kbd "S-<up>") 'drag-stuff-up)

  ;; Move lines down
  (define-key evil-normal-state-map (kbd "S-<down>") 'drag-stuff-down)
  (define-key evil-visual-state-map (kbd "S-<down>") 'drag-stuff-down)
)
